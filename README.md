<<<<<<< HEAD
A. Create the three Marcom Order ReportsNavigate to: https://admin.marcomcentral.app.pti.com/Reports/Edit
(1) Create and Save these report templates, use the following name convention (case, spacing): Job Ticket SummaryTotal Order SummaryOrder And Ship Detail
(2) Modify the columnsFor each saved template, check or activate all available columns.For each saved template, select Sort By Job Ticket Number.
...
<<<<<<< HEAD
C. Run the Script
Navigate to: /Users/~/Desktop/- Controller X.X folder.
=======
 C. Run the Script
